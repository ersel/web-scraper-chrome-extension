var DropDownClick = {
	...
	getClickElements: function(parentElement) {
		var clickElements = ElementQuery(this.dropDownSelector, parentElement);
		var options = [];

		if(clickElements[0]){
			var method = clickElements[0].getAttribute("onchange").slice(0,-1);
			for(var i = 0; i < clickElements[0].options.length; i++){
				options.push(method + clickElements[0].options[i].value + ")");
			}
		}
		return options;
	},
	...
	triggerButtonClick: function(clickElement) {
		...
		script.text  = "" +
			"(function(){ " +
			"var el = document.querySelectorAll('"+this.dropDownSelector+"')[0]; " +
			"el.value=" + value + ";" +
			clickElement +
			"})();";
	...
	getDropDownSelector: function() {

		if(this.dropDownSelector  === undefined) {
			return '';
		}
		else {
	...
	_getData: function(parentElement) {
		...
		var interval = setInterval(function() {
			...
			//console.log("added", elements, addedAnElement);
	...
	getFeatures: function () {
		return ['multiple', 'delay', 'dropDownSelector', 'clickType', 'discardInitialElements']
	}
};
